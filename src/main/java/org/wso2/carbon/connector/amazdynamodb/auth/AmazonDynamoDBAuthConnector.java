	...
	public final void connect(final MessageContext messageContext) {
		...
		try {
			...
			for (Map.Entry<String, String> entry : parametersMap.entrySet()) {
				...
				} else if (entry.getKey().equals("Limit") || entry.getKey().equals("ConsistentRead") ||
				           entry.getKey().equals("ScanIndexForward") || entry.getKey().equals("Segment") ||
				           entry.getKey().equals("TotalSegments")) {
					payloadStrBuilder.append(entry.getValue());
				} else {
					payloadStrBuilder.append('"');
					payloadStrBuilder.append(entry.getValue());
			...
			final byte[] signingKey = getSignatureKey(messageContext, messageContext
					                                          .getProperty(AmazonDynamoDBConstants.SECRET_ACCESS_KEY).toString(), shortDate,
			                                          messageContext.getProperty(AmazonDynamoDBConstants.REGION)
			                                                        .toString(),
			                                          messageContext.getProperty(AmazonDynamoDBConstants.SERVICE)
			                                                        .toString());
		...
		for (final String key : headerKeys) {
			...
			if (messageContext.getProperty(key) != null && !("").equals(messageContext.getProperty(key))) {
				parametersMap.put(namesMap.get(key).toLowerCase(), messageContext.getProperty(key).toString().trim()
				                                                                 .replaceAll(
						                                                                 AmazonDynamoDBConstants.TRIM_SPACE_REGEX,
						                                                                 AmazonDynamoDBConstants.SPACE));
			}
		}
		return parametersMap;
	}
